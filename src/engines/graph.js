...
export default {
  ...
  render({ w, h, p, colors, ink, ground, seed }) {
    ...
    for (const [i, j] of edges) {
      ...
      const ew = Math.max(1.1, 1.1 * U * 0.7);
      const dash = stitch ? { "stroke-dasharray": `${5 * U} ${6 * U}`, stroke: pal[0], "stroke-opacity": 0.6 } : { stroke: ink, "stroke-opacity": 0.4 };
      if (p.curve < 0.02) {
        edgeSvg += el("line", { x1: round(a.x), y1: round(a.y), x2: round(b.x), y2: round(b.y), "stroke-width": ew, "stroke-linecap": "round", ...dash });
      } else {
        ...
        edgeSvg += el("path", { d: smoothPath([[a.x, a.y], [mx, my], [b.x, b.y]], { tension: 0.6 }),
          fill: "none", "stroke-width": ew, "stroke-linecap": "round", ...dash });
      }
    }
    ...
    for (const nd of nodes) {
      nodeSvg += el("circle", { cx: round(nd.x), cy: round(nd.y), r: round(nd.size), fill: nd.c, stroke: ground, "stroke-width": Math.max(1.5, 1.5 * U * 0.6) });
